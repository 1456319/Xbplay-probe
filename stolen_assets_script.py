...
for f in files:
    ...
    report_parts.append(f"- **SHA-256:** `{sha256_hash}`\n")

    if f.endswith(".html"):
        text_content = content.decode('utf-8', errors='ignore')
        if "STREAM_VIEW_URL" in text_content:
            report_parts.append("- Contains placeholder `STREAM_VIEW_URL`.\n")
        if "gssv-play-prodxhome.xboxlive.com" in text_content:
            report_parts.append("- **CRITICAL:** Contains proprietary Microsoft Xbox Cloud Gaming API endpoint (`gssv-play-prodxhome.xboxlive.com`). This strongly indicates unlicensed use of Microsoft's streaming infrastructure.\n")
        if "xSDK client.js" in text_content:
            report_parts.append("- **CRITICAL:** Contains logging strings referencing `xSDK client.js`, suggesting direct copying of Xbox streaming client logic.\n")
    elif f.endswith(".png"):
         report_parts.append("- Raw asset image extracted.\n")
...
with open("stolen_asset_report.md", "w") as report_file:
    report_file.write("".join(report_parts))
