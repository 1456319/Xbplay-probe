...
import constants.GamestreamStartConfig;
import constants.PacketTypes;
import crypto.SgCrypto;
import packet.MessagePacket;
/* loaded from: /app/base.apk/classes5.dex */
public class GamestreamStartMessagePacket extends MessagePacket {
    public byte[] jsonBody;

    @Override // packet.MessagePacket
    public void loadProtectedData() {
        byte[] loadJsonConfig = loadJsonConfig();
        this.jsonBody = loadJsonConfig;
        try {
            writeSgString(loadJsonConfig, this.protectedPayload);
    ...
    private byte[] loadJsonConfig() {
        return GamestreamStartConfig.OPEN_GAMESTREAM.getBytes();
    }

    public GamestreamStartMessagePacket(SgCrypto sgCrypto) {
